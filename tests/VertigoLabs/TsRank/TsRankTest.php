...
class TsRankTest extends BaseORMTestCase
{
    ...
    public function shouldReturnRank($queryStr, $field, $numberFound)
    {
        ...
        static::assertEquals($numberFound, count($results));
        foreach ($results as $result) {
            static::assertArrayHasKey('rank', $result);
        }
    }
    ...
    public function articleProvider()
    {
        return [
            [
                'Test Article One',
                'This is a test article used for running unit tests. It contains a couple keywords such as Elephant, Dolphin, Kitten, Giraffe, and Baboon.'
            ],
            [
            ...
            [
                ...
                'A recent study has shown that giraffes are actually much shorter than researchers previously believed. "What we didn\'t realize was that the giraffes were actually always surrounded by other really short object" says one official.'
            ],
            [
